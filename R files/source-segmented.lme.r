...
#...: argomenti da passare al plot, compresi "col.l" e "lwd.l" che servono per le segmented lines individuali
      ...
      if(!is.null(leg)) legend(leg, legend=paste("id =",id), bty="n")
        ...
        mu<-fitted(obj[[2]])[names(fitted(obj[[2]]))==id] #è fitted.segmented.lme(fit,1)
...
#comunque per level=0 (population parameter) l'identità non vale, ed è necessario fare i calcoli
#   "manualmente"
       obj<-fit[[1]]
       level<-deparse(level)
       switch(level,
...
# -(249/2)*log(2*pi)-.5*determinant(V)$modulus-(t(e)%*%solve(V) %*%e) /2
    ...
    e1<-y1-drop(X1%*%b)
    n<-length(y1)
    #NB sum(log(diag(L))) è uguale a determinant(V, logarithm=TRUE)$modulus/2
...
#metodo: viene passato alla funzione logL. Se 1 la logL che viene calcolata è quella della componente
...
#        browser()
    ...
    if(it.max==0) {
        ...
        #Se fosse "U+G0" darebbe errore perchè G0 non esiste
...
#browser()
    ...
    if(is.null(random.noG)){ #se "random.noG" non è stato specificato in segmented.lme()
        random.noG<-random
        ...
        } else { #SOLO LA FORMULA DOVE C'E' G0 viene aggiornata, se random è una lista di formule 'list(id=pdBlocked(list(pdSymm(~1+weeks), pdSymm(~U-1))))'
